...
/**
 * @brief get the lyric file using the url of the music file
 * @return the path of the lyric file
 */
...
/**
 * @brief get the lyrics by reading from the lyric file
 * @return the lyrics of the music
 * @deprecated reimplemented in bool LyricWidget::processLyricFile(QString filePath)
 */
QString Music::getLyrics() {
    QFile lyricFile(getLyricFile());
    if (lyricFile.exists()) {
    ...
    return QString("Lyrics not available");

}

QString Music::getInfo() const
{
    return title+" - "+author;//+" ["+Common::formatTime(duration)+"]";
}

void Music::detail()
{
    QString info("歌曲名：%1\n艺术家：%2\n时长：%3\n唱片集：%4\n比特率：%5\n文件路径：%6");
    info=info.arg(title,author,Common::formatTime(duration),albumTitle,QString::number(audioBitRate)+"bps",url.toString());
...
QImage Music::getCover() const {
    QMediaPlayer tempPlayer;
    tempPlayer.setMedia(url);
    while(!tempPlayer.isMetaDataAvailable()){
        QCoreApplication::processEvents();
    }
    QImage cover= tempPlayer.metaData(QStringLiteral("ThumbnailImage")).value<QImage>();
    if(cover.isNull()) cover=QImage(":/button_icons/no_music.png");
