...
void MusicList::addMusic(const QList<QUrl> &urls)
{
    ...
    QProgressDialog proDialog("Appending Progress","Cancel",0,urls.size());
    proDialog.setMinimumSize(350,150);
    proDialog.setWindowModality(Qt::WindowModal);
    proDialog.setWindowTitle("Appending, please wait for a moment");
    proDialog.show();
    int x=0;
    foreach (QUrl i, urls) {
        x++;
 ...
 * @deprecated reimplemented in void PlaylistWidget::refresh()
 */
void MusicList::addToListWidget(PlaylistWidget *listWidget)
{
    foreach(const Music &i,music){
        ...
        musicItem->setTitle(i.getInfo());
        musicItem->setCover(i.getCover());

        musicItem->setRow(listWidget->currentRow());
        musicItem->setItem(item);

        listWidget->addItem(item);
        listWidget->setItemWidget(item,musicItem);

        //connect(listWidget,SIGNAL(itemPressed(QListWidgetItem*)), musicItem, SLOT(setItem(QListWidgetItem*)));
